...
const void texNorthway(Pixel **pixels, float **depth_buffer, const Triangle t, const float winding, Pixel **texels, const int tex_height, const int tex_width) {
    ...
    float tu0 = (t.tex[1].u - t.tex[0].u) / (t.v[1].y - t.v[0].y);
    float tu1 = (t.tex[2].u - t.tex[0].u) / (t.v[2].y - t.v[0].y);

    float tv0 = (t.tex[1].v - t.tex[0].v) / (t.v[1].y - t.v[0].y);
    float tv1 = (t.tex[2].v - t.tex[0].v) / (t.v[2].y - t.v[0].y);

    float tw0 = (t.tex[1].w - t.tex[0].w) / (t.v[1].y - t.v[0].y);
    float tw1 = (t.tex[2].w - t.tex[0].w) / (t.v[2].y - t.v[0].y);
...
const void texSouthway(Pixel **pixels, float **depth_buffer, const Triangle t, const float winding, Pixel **texels, const int tex_height, const int tex_width) {
    ...
    float tu1 = (t.tex[2].u - t.tex[0].u) / (t.v[2].y - t.v[0].y);
    float tu2 = (t.tex[2].u - t.tex[1].u) / (t.v[2].y - t.v[1].y);

    float tv1 = (t.tex[2].v - t.tex[0].v) / (t.v[2].y - t.v[0].y);
    float tv2 = (t.tex[2].v - t.tex[1].v) / (t.v[2].y - t.v[1].y);

    float tw1 = (t.tex[2].w - t.tex[0].w) / (t.v[2].y - t.v[0].y);
    float tw2 = (t.tex[2].w - t.tex[1].w) / (t.v[2].y - t.v[1].y);
    ...
    for (float y = y_start; y < y_end; y += 1.0) {
        ...
        float tex_ye = (tv2 * yA) + t.tex[1].v;

        float tex_xs = (tu1 * yA) + t.tex[0].u;
        float tex_xe = (tu2 * yA) + t.tex[1].u;

        float tex_ws = (tw1 * yA) + t.tex[0].w;
        float tex_we = (tw2 * yA) + t.tex[1].w;
